...
{
public:
  SemanticAstar(std::string basis, int num_pol, int deg_pol, double alpha_shrink);
...
private:
  ...
  //
  std::pair<Eigen::Vector3d, std::vector<Eigen::Vector3i>> att_region_;
  std::pair<Eigen::Vector3d, std::vector<Eigen::Vector3i>> rep_region_;
};
