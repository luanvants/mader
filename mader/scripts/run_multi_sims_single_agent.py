...
if __name__ == '__main__':
    ...
    num_of_sims=5;
    total_num_of_obs=50 #100,150,200, 250
    commands = []

    folder_bags="/home/nvhung/Desktop/bags";
    basis="MINVO" #or"MINVO", "BEZIER", "B_SPLINE"
    ...
    for s in range(num_of_sims):
        ...
        commands.append("roslaunch mader mader_specific_and_general.launch gui:=false rviz:=true environment:=false");
        commands.append("sleep 1.0 &&rosrun mader dynamic_corridor.py "+str(total_num_of_obs));
        ...
        commands.append("sleep 5.0 && cd "+folder_bags+" && rosbag record -o "+basis+"_obs_"+str(total_num_of_obs)+"_sim_"+str(s)+" /SQ01s/mader/actual_traj __name:="+name_node_record);
        ...
        session_name="run_multi_sims_single_agent_session"
        ...
        # for i in range(len(commands)):
            # os.system('tmux send-keys -t '+str(session_name)+':0.'+str(i) +' "'+ commands[i]+'" '+' C-m')   #sends the command to the specified pane
            # -t: specifies `str(session_name)` session.
            # `0.` specifies the window (`0`)
            # pane (`i`) within the session.
            # `commands[i]` is the command to be sent to the pane
            # `' C-m'`: pressing the Enter key after the command
        for i in range(len(commands)):
            os.system(f'tmux send-keys -t {session_name}:0.{i} "export ROS_MASTER_URI={os.environ["ROS_MASTER_URI"]} && export PYTHONPATH={os.environ["PYTHONPATH"]} && source /opt/ros/noetic/setup.bash && source ~/ws_mader/devel/setup.bash && {commands[i]}" C-m')
