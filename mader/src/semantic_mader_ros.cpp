...
#include "semantic_mader_ros.hpp"
...
MaderRos::MaderRos(ros::NodeHandle nh1, ros::NodeHandle nh2, ros::NodeHandle nh3) : nh1_(nh1), nh2_(nh2), nh3_(nh3)
{
  ...
  mu::safeGetParam(nh1_, "att_weight", par_.att_weight);
  mu::safeGetParam(nh1_, "rep_weight", par_.rep_weight);
  ...
  pub_actual_traj_ = nh1_.advertise<visualization_msgs::Marker>("sem_actual_traj", 1);
  ...
  sub_semantic_area_ = nh1_.subscribe("/grass_marker", 1, &MaderRos::semaAreaCB, this);
...
void MaderRos::semaAreaCB(const visualization_msgs::Marker& msg) {
    // Extract the position of the marker
    Eigen::Vector3d position(msg.pose.position.x, msg.pose.position.y, msg.pose.position.z);

    // Extract the scale (dimensions) of the marker
    Eigen::Vector3d scale(msg.scale.x, msg.scale.y, msg.scale.z);

    // Generate the voxelized region
    std::vector<Eigen::Vector3i> voxel_region;

    // Define the voxel size (resolution of the grid)
    double voxel_size = 0.1;  // Adjust as needed

    // Calculate the bounding box corners
    Eigen::Vector3d min_corner = position - scale / 2.0;
    Eigen::Vector3d max_corner = position + scale / 2.0;

    // Loop through the bounding box and generate voxel indices
    for (double x = min_corner.x(); x <= max_corner.x(); x += voxel_size) {
        for (double y = min_corner.y(); y <= max_corner.y(); y += voxel_size) {
            for (double z = min_corner.z(); z <= max_corner.z(); z += voxel_size) {
                Eigen::Vector3i voxel_index(
                    static_cast<int>(std::floor(x / voxel_size)),
                    static_cast<int>(std::floor(y / voxel_size)),
                    static_cast<int>(std::floor(z / voxel_size))
                );
                voxel_region.push_back(voxel_index);
            }
        }
    }

    // Store the position and voxelized region in att_region_
    // att_region_ = std::make_pair(position, voxel_region);

    // Debug output (optional)
    // ROS_INFO("Attractive region extracted: Position (%f, %f, %f), Voxel count: %lu",
            //  position.x(), position.y(), position.z(), voxel_region.size());
}
...
void MaderRos::whoPlansCB(const mader_msgs::WhoPlans& msg)
{
  ...
  {  // PANTHER is the one who plans now (this happens when the take-off is finished)
    ...
    std::cout << on_blue << "**************SEMANTIC MADER STARTED" << reset << std::endl;
  }
}
...
{
  //kiem tra ham nay chay sau khi cap '2D Nav Goal' trong RVIZ
  // ROS_INFO("terminalGoalCB triggered: Received goal at (%f, %f, %f)",
