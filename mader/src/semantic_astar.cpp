...
SemanticAstar::SemanticAstar(std::string basis, int num_pol, int deg_pol, double alpha_shrink)
{
  p_ = deg_pol;
  M_ = num_pol + 2 * p_;
  N_ = M_ - p_ - 1;
  ...
  if (basis == "MINVO")
  {
    // std::cout << green << bold << "Semantic A* is using MINVO" << reset << std::endl;
  ...
  {
    // std::cout << green << bold << "Semantic A* is using BEZIER" << reset << std::endl;
  ...
  {
    // std::cout << green << bold << "Semantic A* is using B_SPLINE" << reset << std::endl;
...
void SemanticAstar::setUp(double t_min, double t_max, const mt::ConvexHullsOfCurves_Std& hulls)
{
...
SemanticAstar::~SemanticAstar()
{
}

int SemanticAstar::getNumOfLPsRun()
{
  return separator_solver_->getNumOfLPsRun();
}

void SemanticAstar::setVisual(bool visual)
{
  visual_ = visual;
}

void SemanticAstar::getBestTrajFound(mt::trajectory& best_traj_found, mt::PieceWisePol& pwp, double dc)
{
  mt::trajectory traj;
  CPs2TrajAndPwp(result_, best_traj_found, pwp, N_, p_, num_pol_, knots_, dc);
}

void SemanticAstar::getEdgesConvexHulls(mt::Edges& edges_convex_hulls)
{
...
void SemanticAstar::getAllTrajsFound(std::vector<mt::trajectory>& all_trajs_found)
{
...
void SemanticAstar::setBBoxSearch(double x, double y, double z)
{
  bbox_x_ = x;
  bbox_y_ = y;
  bbox_z_ = z;
...
void SemanticAstar::setMaxValuesAndSamples(Eigen::Vector3d& v_max, Eigen::Vector3d& a_max, int num_samples_x,
...
void SemanticAstar::setXYZMinMaxAndRa(double x_min, double x_max, double y_min, double y_max, double z_min,
...
void SemanticAstar::setBias(double bias)
{
  bias_ = bias;
}

//
void SemanticAstar::setAttWeight(double attweight) {
  att_weight_ = attweight;
}
//
void SemanticAstar::setRepWeight(double repweight) {
  rep_weight_ = repweight;
}
//
...
void SemanticAstar::setRunTime(double max_runtime)
{
  max_runtime_ = max_runtime;
}
void SemanticAstar::setGoalSize(double goal_size)
{
...
void SemanticAstar::computeLimitsVoxelSize(double& min_voxel_size, double& max_voxel_size)
{
  int i = 2;
...
// // axis=0 (x), 1(y) or 2(z)
// bool SemanticAstar::computeAxisForNextInterval(const int i, const Eigen::Vector3d& viM1, int axis, double& constraint_L,
//                                                double& constraint_U)
// {
...
bool SemanticAstar::computeUpperAndLowerConstraints(const int i, const Eigen::Vector3d& qiM2,
...
void SemanticAstar::computeInverses()
{
  Ainverses_.clear();
...
void SemanticAstar::setq0q1q2(Eigen::Vector3d& q0, Eigen::Vector3d& q1, Eigen::Vector3d& q2)
{
  q0_ = q0;
  q1_ = q1;
  q2_ = q2;
...
double SemanticAstar::h(Node& node)
{
  return (node.qi - goal_).norm();
}

double SemanticAstar::g(Node& node)
{
  double cost = 0;
  Node* tmp = &node;
...
double SemanticAstar::weightEdge(Node& node1, Node& node2)  // edge cost when adding node 2
{
...
void SemanticAstar::recoverPath(Node* result_ptr)
{
...
Eigen::Matrix<double, 3, 4> SemanticAstar::transformBSpline2otherBasis(const Eigen::Matrix<double, 3, 4>& Qbs,
                                                                       int interval)
{
  return Qbs * M_pos_bs2basis_[interval];
}

Eigen::Matrix<double, 3, 4> SemanticAstar::transformOtherBasis2BSpline(const Eigen::Matrix<double, 3, 4>& Qmv,
                                                                       int interval)
{
  return Qmv * M_pos_bs2basis_inverse_[interval];
}

bool SemanticAstar::checkFeasAndFillND(std::vector<Eigen::Vector3d>& q, std::vector<Eigen::Vector3d>& n,
                                       std::vector<double>& d)
{
  ...
  for (int index_interv = 0; index_interv < (q.size() - 3); index_interv++)
  {
    ...
    for (int obst_index = 0; obst_index < num_of_obst_; obst_index++)
    {
      ...
      if (solved == false)
      {
        ...
        // std::cout << bold << red << "[Semantic A*] The node provided doesn't satisfy LPs" << reset << std::endl;
...
bool SemanticAstar::collidesWithObstacles(Node& current)
{
  Eigen::Matrix<double, 3, 4> last4Cps;  // Each column contains a control point
...
void SemanticAstar::expandAndAddToQueue(Node& current, double constraint_xL, double constraint_xU, double constraint_yL,
                                        double constraint_yU, double constraint_zL, double constraint_zU)
{
...
bool SemanticAstar::collidesWithObstaclesGivenVertexes(const Eigen::Matrix<double, 3, 4>& last4Cps, int index_lastCP)
{
...
bool SemanticAstar::run(std::vector<Eigen::Vector3d>& result, std::vector<Eigen::Vector3d>& n, std::vector<double>& d)
{
  /////////// reset some stuff
  ...
  comparer.bias = bias_;
  comparer.att_weight = att_weight_;
  comparer.rep_weight = rep_weight_;

  //
  std::priority_queue<Node, std::vector<Node>, CompareCost> openList_tmp(comparer);
  openList_ = openList_tmp;

  std::cout << "[Semantic A*] Running..." << std::endl;
  ...
  while (openList_.size() > 0)
  {
    ...
    {
      std::cout << "[Semantic A*] Max Runtime was reached" << std::endl;
    ...
    *current_ptr = openList_.top();  // copy the first element onto (*current_ptr)
    ...
    {
      std::cout << "[Semantic A*] Goal was reached!" << std::endl;
      status = GOAL_REACHED;
      goto exitloop;
    }
    if ((*current_ptr).index == (N_ - 2))
  ...
  std::cout << "[Semantic A*] openList_ is empty" << std::endl;
...
exitloop:
  ...
  {
    std::cout << "[Semantic A*] choosing current_ptr as solution" << std::endl;
  ...
  else if ((status == RUNTIME_REACHED || status == EMPTY_OPENLIST) && have_a_solution)
  {
    ...
    if (complete_closest_result_so_far_ptr_ != NULL)
    {
      std::cout << "[Semantic A*] choosing closest complete path as solution" << std::endl;
      best_node_ptr = complete_closest_result_so_far_ptr_;
      std::cout << bold << blue << "[Semantic A*] complete_closest_dist_so_far_= " << complete_closest_dist_so_far_ << reset
                << std::endl;
    }
    else
    {
      std::cout << "[Semantic A*] choosing closest path as solution" << std::endl;
