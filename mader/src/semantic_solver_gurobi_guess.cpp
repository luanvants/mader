...
#include "semantic_solver_gurobi.hpp"
...
bool SolverGurobi::generateAStarGuess()
{
  ...
  semanticAstarSolver_->setUp(t_init_, t_final_, hulls_);

  semanticAstarSolver_->setq0q1q2(q0_, q1_, q2_);
  semanticAstarSolver_->setGoal(final_state_.pos);

  double goal_size = 0.05;  //[meters]

  semanticAstarSolver_->setXYZMinMaxAndRa(par_.x_min, par_.x_max, par_.y_min, par_.y_max, par_.z_min, par_.z_max,
                                    par_.Ra);             // limits for the search, in world frame
  semanticAstarSolver_->setBBoxSearch(2000.0, 2000.0, 2000.0);  // limits for the search, centered on q2
  semanticAstarSolver_->setMaxValuesAndSamples(par_.v_max, par_.a_max, par_.a_star_samp_x, par_.a_star_samp_y,
                                         par_.a_star_samp_z, par_.a_star_fraction_voxel_size);

  semanticAstarSolver_->setRunTime(kappa_ * max_runtime_);  // hack, should be kappa_ * max_runtime_
  semanticAstarSolver_->setGoalSize(goal_size);
  semanticAstarSolver_->setBias(par_.a_star_bias);
  semanticAstarSolver_->setVisual(false);

  // set attraction and repulsion regions
  // semanticAstarSolver_->setAttWeight(par_.att_weight);
  // semanticAstarSolver_->setRepWeight(par_.rep_weight);
  semanticAstarSolver_->setAttWeight(2.0);

  //
  double voxel_size = semanticAstarSolver_->getVoxelSize();
  Eigen::Vector3d region_origin(10.0, 3.0, 0.0);
  double region_bbox_x = 10.0;
  double region_bbox_y = 2.0;
  ...
  semanticAstarSolver_->setAttRegion(att_region);

  //
  std::vector<Eigen::Vector3d> q;
  std::vector<Eigen::Vector3d> n;
  std::vector<double> d;
  bool is_feasible = semanticAstarSolver_->run(q, n, d);

  // num_of_LPs_run_ = semanticAstarSolver_->getNumOfLPsRun();
...
Eigen::Vector3i SolverGurobi::getNodeVoxelIndex(const Eigen::Vector3d& node_qi, double voxel_size, const Eigen::Vector3d& voxel_origin) {
